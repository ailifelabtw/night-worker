...
當使用者說想要「夜間工人團隊」幫他做事，或丟一個具體任務 / 問題 / 困擾過來，你要：

### 適合丟給夜間工人團隊的事（給使用者參考）

- 💡 **明天可以發文的主題 + 完整文案**（IG / Threads / FB / blog）
- 📄 **改履歷**（給 1-2 個職位 JD + 現有履歷，讓三個模型辯論該怎麼改）
- 🤔 **A vs B vs C 方案哪個值得做**（譬如：接這個 case / 換工作 / 報這個課 / 買哪台筆電）
- 📚 **長文／email／提案的開場怎麼寫**（給背景 + 想達到的效果，讓他們腦力激盪幾個版本）
- 🎯 **下一季 / 下個月該主推什麼**（給目前狀況 + 目標 + 限制，讓他們提幾條路線）
- 💔 **不太知道怎麼開口的對話**（譬如：跟伴侶冷戰想和好、跟老闆提離職、跟家人講壞消息——三個模型分別模擬不同角度）
- 🧭 **遇到模糊 / 兩難情境想要第三方意見**（譬如：「客戶想要我做 X 但我覺得會踩雷」）
...
1. **任務是什麼？**（如果他講得不夠具體再追問 1 個問題）
2. **有沒有背景資料 / 已知的選項 / 紅線**？（沒有也 OK，他可能就是想要你開腦洞；有的話接過來放進 `current_focus`）
3. **想要哪種辯論模式**？給三個簡述讓他選：
   - 共識型（廣撒網提案、收斂出最佳解）— 適合「我不知道有哪些選項」
   - 對抗型（一方提案、一方專挑翻車情境）— 適合「我已經有方向但怕踩雷」
   - 文案職人型（直接寫完整貼文／文案）— 適合「我要寫 IG/Threads/blog/履歷」
